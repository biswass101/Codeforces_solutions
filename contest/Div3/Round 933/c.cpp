...
int countSubstr(const string& x, const string & substr) {
    int count = 0;
    size_t pos = x.find(substr); // Find the first occurrence of substr

    while (pos != string::npos) { // Loop until no further occurrence is found
        count++; // Increment count
        pos = x.find(substr, pos + 1); // Find the next occurrence of substr
    }
