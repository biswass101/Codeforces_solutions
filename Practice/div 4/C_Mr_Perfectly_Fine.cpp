...
void solve()
{
    ...
    int mn_01 = imax, mn_10 = imax, mn_11 = imax; // imax, 9, 8
    fr(i, 0, n, 1)
    {
        if(v[i].second == "01") mn_01 = min(mn_01, v[i].first);
        if(v[i].second == "10") mn_10 = min(mn_10, v[i].first);
        if(v[i].second == "11") mn_11 = min(mn_11, v[i].first);
    }
    int combine = -1;
    if(mn_01 != imax and mn_10 != imax) combine = mn_01 + mn_10;

    if(combine == -1 and mn_11 == imax) cout << -1 << el;
    else if(combine == -1 and mn_11 != imax) cout << mn_11 << el;
    else cout << min(combine, mn_11) << el;
}
signed main()
{
#ifndef OJ
